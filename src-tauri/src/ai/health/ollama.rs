//! Ollama health probe.
//!
//! Ollama runs as a local HTTP server. `GET {base}/api/tags` succeeds (200)
//! whenever the daemon is up, so we use it as the liveness check. A connection
//! error (daemon not running) degrades to `ok: false` with a clear message so
//! the UI shows "unavailable" rather than a confusing auth/binary fallback.

use std::time::Duration;
use crate::ai::process::ollama::DEFAULT_BASE_URL;
    ...
    match client.get(&url).send().await {
        Ok(resp) if resp.status().is_success() => {
            let count = resp
                ...
                .await
                .ok()
                .map(|v| crate::ai::process::normalizer::parse_ollama_tags(&v).len())
                .unwrap_or(0);
            HealthStatus {
                ok: true,
